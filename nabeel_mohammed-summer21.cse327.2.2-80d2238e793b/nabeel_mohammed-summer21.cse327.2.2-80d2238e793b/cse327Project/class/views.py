...
from .models import teacher,student,Class,Material,Question,Image,readingReview
...
def pdfview(request,Mid):
    ...
    if request.method=="POST":
        if "question" in request.POST:
            ...
            q=Question.objects.create(question=q,student_id=s,page_no=pno,width=request.POST.get('width'),top=request.POST.get('top'),left=request.POST.get('left'),material_id=material)#new text question
            q.save()
            ...
            if(readingReview.objects.filter(studentId=data3,materialId=M,status='incomplete')):
                ...
                readingReview.objects.filter(studentId=data3, materialId=M, status='incomplete').update(comTime=timezone.now(),status='complete')
            ...
            s = student.objects.get(id=request.session.get('student_id'))  # get student information
            material = Material.objects.get(id=Mid)  # get material information
            q = Question.objects.create(AudioQuestion=audio, student_id=s, page_no=pno,
